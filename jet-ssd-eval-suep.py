...
if __name__ == '__main__':
    ...
    parser.add_argument('-twn', type=str, default=None, required=False,
                        help='Ternary Weight Network model name')
    parser.add_argument('-int8', type=str, default=None, required=False,
    ...
    ### FIXME: temporarly removed baseline
    ...
    base_results, base_deltas = None, None

    for i, name in enumerate([args.fpn, args.twn, args.int8]):
        
        if name is None: 
            plotting_results.append(None)
            plotting_deltas.append(None)
            continue
    ...
    for i, (jap, jpr3, jpr5, name) in enumerate(zip(ap, pr3, pr5, models)):
        if i == 0 and base_results is None: continue
        elif i > 0 and plotting_results[i-1] is None: continue
